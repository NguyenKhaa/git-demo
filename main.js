...
trim()                    xóa bỏ khoảng trắng 2 bên của value
----*/

let form = document.querySelector("form");

form.addEventListener("submit", function (event) { // thêm sự kiện vào form
  let inputValue = document.getElementById("txt").value; //lấy ra giá trị input
  event.preventDefault();
  let val = inputValue.trim();
  if (val) { // nếu có giá trị thì gọi tới hàm addTask
    createTask();
  } else {
